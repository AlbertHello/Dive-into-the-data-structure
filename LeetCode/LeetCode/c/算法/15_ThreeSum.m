...
-(NSArray *)twoSum1:(NSArray *)nums target:(NSInteger)target{
    for (int i = 0; i < nums.count - 1; i++) { // 每个人
        for (int j = i + 1; j < nums.count; j++) { // 都去问其他的人
...
-(NSArray *)twoSum2:(NSArray *)nums target:(NSInteger)target{
    ...
    for (int j = 0; j < nums.count; j++) {
        ...
        if ([map.allKeys containsObject:matchKey]) {
...
-(NSArray *)threeSum1:(NSArray *)nums target:(NSInteger)target{
    for (int i = 0; i < nums.count - 2; i++) { // 每个人
        for (int j = i + 1; j < nums.count-1; j++) { // 都去问其他的人
...
/**
 ...
 复杂度分析
 1、时间复杂度：O(n^2)，数组排序O(NlogN)，遍历数组O(n)，双指针遍历O(n)，总体O(NlogN)+O(n)∗O(n)，O(n^2)
 2、空间复杂度：O(1)
 
 */
+(NSArray *)threeSum2:(NSArray *)nums target:(NSInteger)target{
    //对于数组长度n，如果数组为null 或者数组长度小于3，返回[]。
